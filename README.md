...
- 右栏下拉框选择 agent 与模型商,输入问题,Enter 发送;回复期间"发送"按钮置灰禁用
- pi 的回复为流式输出:思考过程实时展开,开始回答时自动收起为可点击的折叠块,回答逐字流出,随后生成幻灯片
...
- **⤓ PDF**:在当前页面注入打印样式并唤起打印对话框,选"另存为 PDF"即可——课堂上 AI 生成的动态幻灯片会一起导出;表格在 PDF 中完整展开。打印完成后页面自动恢复正常视图
- **⤓ 问答**:把本次的提问、AI 回答、以及生成的幻灯片完整内容导出为 Markdown 文件下载

### 对话栏收放

slide 右边界顶部的页签按钮(» / «)可收起/展开对话栏。收起后幻灯片切换为 16:9 铺满整个窗口,展开时恢复左侧 4:3 课件 + 右侧对话栏。
